...
class AllTasksTeam extends StatefulWidget {

  AllTasksTeam({Key? key, this.taskFilter}) : super(key: key);
  final taskFilter;
  @override
  _AllTasksTeamState createState() => _AllTasksTeamState();
}


class _AllTasksTeamState extends State<AllTasksTeam> {
  ...
  void getLoggedInUserDetails() async {
    ...
    print("ewfwfe data is ${x[0].data()['department'][0]}");
  }
...
var teamAllTasks = FirebaseFirestore.instance.collection('spark_assignedTasks').where("to_email", isEqualTo: _auth.currentUser?.email).snapshots();
var personalDoneTasks = FirebaseFirestore.instance.collection('spark_assignedTasks').where("status", isEqualTo: "Done").where("to_email", isEqualTo: _auth.currentUser?.email).snapshots();
var personalCreatedTasks = FirebaseFirestore.instance.collection('spark_assignedTasks').where("by_email", isEqualTo: _auth.currentUser?.email).snapshots();



Widget _personalAllTasks(dept) => StreamBuilder<QuerySnapshot>(
  stream: FirebaseFirestore.instance.collection('spark_assignedTasks').where("dept", isEqualTo: dept).snapshots(),
